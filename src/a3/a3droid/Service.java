...
public class Service extends HandlerThread implements BusObject, A3ServiceInterface, UserInterface, TimerInterface{
	...
	public void connect(){
		...
		if (status == Status.OK) {
			...
			if (status != Status.OK) {
				...
				try{
					sessionLessSignalEmitter = new SignalEmitter(this, 0, SignalEmitter.GlobalBroadcast.Off);
					sessionLessSignalEmitter.setSessionlessFlag(true);
	    			sessionLessSignalEmitterInterface = sessionLessSignalEmitter.getInterface(A3ServiceInterface.class);
				}catch(Exception e){
	...
	public void receivedGroupIsDuplicated(String groupName, String supervisorId, int numberOfNodes, int randomNumber) {
	
		if(groupName.equals(this.groupName) && !supervisorId.equals(this.supervisorId)){
	...
	public void sendDuplicatedGroupSignal(int randomNumber){
		try {
			sessionLessSignalEmitterInterface.GroupIsDuplicated(groupName, supervisorId, view.getNumberOfNodes(), randomNumber);
		} catch (BusException e) {
			// TODO Auto-generated catch block
			showOnScreen("ECCEZIONE IN sendDuplicatedGroupSignal(): " + e.getLocalizedMessage());
		}
	}
